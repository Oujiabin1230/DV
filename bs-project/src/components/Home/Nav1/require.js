...
export async function Nav1_SubNav1_Data(searchDate,searchCount) {
   var url = `http://8.134.93.160:8000/api/total_cases/?start_date=${searchDate}&end_date=${searchDate}&country_count=${searchCount}`
...
export async function Nav1_SubNav2_Data(searchDate,searchCount) {
   var url = `http://8.134.93.160:8000/api/total_deaths/?start_date=${searchDate}&end_date=${searchDate}&country_count=${searchCount}`
   const { data: res, status, statusText } = await axios.get(url)
   if (status !== 200) {
      ElMessage.error(statusText)
    }
...
export async function Nav1_SubNav3_Data(searchDate,searchCount) {
   var url = `http://8.134.93.160:8000/api/total_tests/?start_date=${searchDate}&end_date=${searchDate}&country_count=${searchCount}`
   const { data: res, status, statusText } = await axios.get(url)
   if (status !== 200) {
      ElMessage.error(statusText)
    }
...
export async function Nav1_SubNav4_Data(searchDate,searchCount) {
   var url = `http://8.134.93.160:8000/api/new_cases_sum/?start_date=${searchDate[0]}&end_date=${searchDate[1]}&country_count=${searchCount}`
   const { data: res, status, statusText } = await axios.get(url)
   if (status !== 200) {
      ElMessage.error(statusText)
    }
...
export async function Nav1_SubNav5_Data(searchDate,searchCount) {
   var url = `http://8.134.93.160:8000/api/new_deaths_sum/?start_date=${searchDate[0]}&end_date=${searchDate[1]}&country_count=${searchCount}`
   const { data: res, status, statusText } = await axios.get(url)
   if (status !== 200) {
      ElMessage.error(statusText)
    }
   ...
   let title = res.respdata.sql_name
   let response_name = res.respdata.国家
   let response_num = res.respdata.新增死亡人数
   let introduce = `${searchDate},${searchCount}`
   // 此处为示例数据
   var option = {
      color:'red',
